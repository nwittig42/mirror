...
|---|---|---|
| **Citation rate** | 50% | % of tracked prompt-checks where the brand is named, **excluding branded prompts** (any engine) |
| **Position quality** | 20% | Named first / in top 3 / mentioned / absent (weighted) on "best of" prompts |
| **Coverage breadth** | 15% | On how many of the 4 engines they appear **on a prompt that didn't name them** (an 80-on-Perplexity, 0-elsewhere practice is fragile) |
...
**Why branded prompts are excluded from the two visibility components.** A branded
prompt names the practice in the question ("How much does Botox cost at Glow MedSpa?"),
so the answer names it back essentially every time, for every practice, visible or not.
Counting that as a citation measures nothing and inflates the headline: on the first two
practices we scanned it was 6/6 and 6/6, worth roughly 13 points of citation rate that
had not been earned. Branded prompts still do real work, but it is accuracy work: they
are how the judge catches hallucinations, and they are the entire denominator of the
accuracy component below. They are counted once, where they belong, rather than twice.

A consequence worth saying out loud at kickoff: a practice that is genuinely never named
in its own niche now scores 0, not 25. That is the correct reading, and it is the number
the "Invisible" band was written for.
